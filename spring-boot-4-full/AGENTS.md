...
**Devcontainer change, same session:** `~/devbox/.devcontainer/compose.yml` now has a `claude-config` named volume on the `workspace` service (mirrors the existing `rovodev-config` pattern) mounted at `~/.claude`, so credentials/session history/memory files survive a container rebuild going forward. Being a *new* volume, Docker would otherwise start it empty on first creation (it only auto-populates from the image's own baked-in content, not a currently-running container's writable layer) — so the pre-volume state was bridged across that one gap: `~/devbox/.claude-state-backup.tar.gz` (history/memory/settings, **`.credentials.json` deliberately excluded** — never archived to a plain file) plus a one-time restore step added to `post-create.sh` (only fires if `~/.claude/history.jsonl` is absent, i.e. only on that volume's actual first creation; never overwrites real accumulated state on any later rebuild). `post-create.sh`'s existing bind-mount ownership fix (Docker creates mount targets root:root before the container user exists) was also extended to cover `~/.claude`, since the same issue would otherwise hit the new volume too. Net effect: the very next rebuild restores everything except the login — expect to re-authenticate once, after which the volume is real and self-sustaining.

**Compose consolidation, same session — the per-module standalone compose files are gone.** `auth-server/compose.yaml` and `resource-server/compose.yaml` (their own MySQL containers, `authuser`/`resourceuser` credentials on `localhost:3307`/`3308`, plus auth-server's own Keycloak/Mailpit) were **deleted**. Everything they provided now lives in one place:

- **`~/repos/compose.yml`** (new) — the canonical stack: `mysql-auth`, `mysql-resource`, `keycloak`, `mailpit`. Runnable standalone, no devcontainer: `cd ~/repos && docker compose up -d`. `mysql-auth`/`mysql-resource` now publish a *second* host port each — `3307`/`3308` alongside the existing `13306`/`13307` — listening on the same internal port both times, not remapped. `mysql/auth-init.sql`/`mysql/resource-init.sql` (moved here from `.devcontainer/mysql/`) now also `CREATE USER` `authuser`/`resourceuser` (matching `application.yaml`'s hardcoded DEFAULT-profile credentials) alongside the existing devpod-profile `dev` user, on the same databases. Net effect: `auth-server`/`resource-server`'s DEFAULT profile (no active profile — what IntelliJ uses by default) now resolves directly against this same devbox stack, unchanged, no env vars or profile flags needed. **Applied live against the already-running containers this session** (`CREATE USER`, via `mysql` client — confirmed working); the compose port publishing itself needs one `docker compose up` from the host to take effect, since I can't run that from in here.
- **`~/devbox/.devcontainer/compose.yml`** (slimmed) — now just `include: - path: ${REPOS_DIR:-${HOME}/repos}/compose.yml` plus the `workspace` service and its own volumes (`rovodev-config`, `jetbrains-cache`, `claude-config`). `devcontainer.json`'s `dockerComposeFile: compose.yml` reference needed no change — it still points at this same file, which now just does more via `include:`. Both entry points share `name: spring-security-lab`, so they attach to one stack rather than starting a parallel second one.
- **Keycloak realm de-duplicated too**: `.devcontainer/keycloak/lab-realm.json` (a byte-identical copy of `auth-server/docker/keycloak/lab-realm.json`) is gone; `~/repos/compose.yml`'s `keycloak` service now mounts `auth-server/docker/keycloak/` directly. `README.md`'s "Keycloak realm" section and `PLAN.md`'s Phase 7 note were updated to the new path — the old `.devcontainer/keycloak/` instruction would have been silently wrong otherwise.
- **Validated with `docker compose config`** (client-side only, no daemon needed — confirmed that works in this sandbox) from both entry points: `cd ~/repos && docker compose config` and, forcing `REPOS_DIR` to the container-visible path since the real value is host-only, `cd ~/devbox/.devcontainer && REPOS_DIR=/home/vscode/repos docker compose config` — both parse clean, and the overlay's `workspace.depends_on` correctly resolves against the included `mysql-auth`/`mysql-resource` services. **Not validated**: an actual `docker compose up` of either entry point, since the daemon issue noted above blocks that from this sandbox — worth a real end-to-end run from the host before trusting this fully.
- **Follow-up, same session**: `docker/` (repo root) now holds both `docker/mysql/` (`auth-init.sql`/`resource-init.sql`, moved from a short-lived top-level `mysql/`) and `docker/keycloak/` (moved from `auth-server/docker/keycloak/`, itself a first pass at de-duplicating the old `.devcontainer/keycloak/` copy). `auth-server/docker/` and `resource-server/docker/` (the latter always empty — an unused `mysql/init/` referenced by the now-deleted standalone compose) are gone entirely. `~/repos/compose.yml` and every doc/comment referencing the old paths (`README.md`, `PLAN.md`, `application.yaml`, `DataInitializer.java`) updated to match; re-validated with `docker compose config` again after the move.
